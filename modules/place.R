...
place_server <- function(id, place_type) {
  moduleServer(id, function(input, output, session){
    ...
    place_list <- NULL
    for (c in sort(unique(places$category))) {
    ...
    output$place <- renderUI({
      tagList(
        br(),
        fluidRow(column(12, selectInput(ns("place_name"), label="Select Regional Growth Center:", choices=place_list, selected = random_place, width = '100%'))),
